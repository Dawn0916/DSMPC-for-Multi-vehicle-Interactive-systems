function MovingTrajectory_car1View(states_vehicles,params_lane,predicted_states_EV,colors_vehicles,iterations)
...
states_vehicle_TV=states_vehicles{2}(end,:);
...
TV1=plot(position_x_TV', position_y_TV','x','linewidth',3,'color',colors_vehicles(2,:));
hold on
...
% assumeded_states_TV=states_vehicles{2}(end,:);
...
title(sprintf('Moving trajectories from the view of vehicle 1'),'interpreter','latex','FontSize',fontsize_labels)
...
saveas(gcf,sprintf('Moving Trajectories from the view of vehicle 1 FIG%d.fig',iterations_step));
saveas(gcf,sprintf('MovingTrajectoryOfvehicle1FIG%d.png',iterations_step));
set(EV1,'Visible','off');
% if isempty(assumeded_states_TV{2})==0
% set(TV2,'Visible','off');
% else
...
% else
% end
% saveas(gcf,'Moving Trajectories from the view of vehicle 1-FIG%d',iterations_step,'fig');
% saveas(gcf,'MovingTrajectoryOfvehicle1-FIG%d',iterations_step,'png');
end
