...
if ErrorFlag(simu)==0
 ...
 for i=2:number_vehicles
    ...
    grid minor
    hold on
    saveas(gcf,'single_SMPC_Collisions_Without_Error','fig')
    saveas(gcf,'single_SMPC_Collisions_Without_Error','png')
 save('Collisions_record_100_Method1_NoError.mat','Collisions_record');
 Collisions_record_method1=Collisions_record;
 CollosionStep=0;
 for simu=1:simulation_repeat
     CollosionStep_simu=length(Collisions_record_method1{simu});
     CollosionStep=CollosionStep+CollosionStep_simu;
 end
AverageCollisionStep=CollosionStep/(simulation_repeat-ErrorNum)
ErrorNum
