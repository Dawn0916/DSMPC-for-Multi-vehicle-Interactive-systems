function [Flag_Error_f,Flag_c,predicted_states_EV, t, x, u] = nmpc(runningcosts, terminalcosts, ...
...
% (C) Lars Gruene, Juergen Pannek 2011
    extra_params=11; % extra function inputs compared to original nmpc.m
...
%         fprintf('MATLAB version R2008 or newer detected\n');
    ...
    while(mpciter < mpciterations)
        %%% Flag for collision avoidance
        Flag_constraints=0;
        Flag_terminalconstraints=0;
        Flag_chance_constraints=0;
        
        
        
        %%% Print time step t   
        fprintf('Real/iteration time step: \n');
        fprintf('%d \n', t);
        ...
        [u_new, V_current, exitflag, output] = solveOptimalControlProblem ...
            ...
            terminalconstraints,chance_constraints, linearconstraints, system, ...
...
%         end
        ...
        %   Print solution
        if ( iprint >= 1 )
            Flag_Error_f=printSolution(system, printHeader, printClosedloopData, ...
        ...
        x_predict_EV = computeOpenloopSolution(system,id_vehicle,states_vehicles, N, T, t0, x0, u, ...
        ...
        %%% print Stage constraints
        for k=1:N
            [cnew_p, ceqnew_p] = constraints_p(t0+k*T,x_p(k,:),u_p(:,k),(k)*T, params_vehicles, params_lane);
            c_p = [c_p cnew_p];
            ceq_p = [ceq_p ceqnew_p];
            Find_negative_positions_constraints=find(c_p>0);
            Find_negative_number_constraints=length(Find_negative_positions_constraints);
            Flag_constraints=Flag_constraints+Find_negative_number_constraints;
        end
        
        %%% print Terminal constraints
        [cnew_p, ceqnew_p] = terminalconstraints_p(t0+(N+1)*T,x_p(N+1,:),N*T, params_vehicles, params_lane);
        c_p = [c_p cnew_p];
        ceq_p = [ceq_p ceqnew_p];
        Find_negative_positions_terminalconstraints=find(cnew_p>0);
        Find_negative_number_terminalconstraints=length(Find_negative_positions_terminalconstraints);
        Flag_terminalconstraints=Flag_terminalconstraints+Find_negative_number_terminalconstraints;

        %%% print chance constraints
        [cnew_p, ceqnew_p] = chance_constraints_p(x_p,u_p,T,id_vehicle, params_vehicles,number_vehicles,id_lane_vehicles,distance_detectable,N,risk_parameter,states_vehicles);   % Chance constraints to avoid collision
        c_p = [c_p cnew_p];
        ceq_p = [ceq_p ceqnew_p];
        Find_negative_positions_chance_constraints=find(cnew_p>0);
        Find_negative_number_chance_constraints=length(Find_negative_positions_chance_constraints);
        Flag_chance_constraints=Flag_chance_constraints+Find_negative_number_chance_constraints;
      
        Flag_c{id_vehicle}=[Flag_constraints Flag_terminalconstraints Flag_chance_constraints];
    
%         %%% print
%         fprintf('\n');
%         fprintf('chance constraints at current time step: \n');
%         fprintf('%d \n', cnew_p);
...
function [u, V, exitflag, output] = solveOptimalControlProblem ...
    ...
    linearconstraints, system, id_vehicle, params_vehicles, params_lane, states_ref,number_vehicles,id_lane_vehicles,distance_detectable,risk_parameter,states_vehicles, N, t0, x0, u0, T, f, A_d, B_d,...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)
...
%     [f, A_d, B_d]=fAB(T);
    ...
    [u, V, exitflag, output] = fmincon(@(u) costfunction(runningcosts, ...
        ...
        system, id_vehicle, params_vehicles, params_lane, number_vehicles,id_lane_vehicles,distance_detectable,risk_parameter,states_vehicles, N, T, t0, x0, u, f, A_d, B_d, ...
...
function [c,ceq] = nonlinearconstraints(constraints, ...
    ...
    id_vehicle, params_vehicles, params_lane,number_vehicles,id_lane_vehicles,distance_detectable,risk_parameter,states_vehicles,  N, T, t0, x0, u, f, A_d, B_d, atol_ode_sim, rtol_ode_sim, type)
                     % id of vehicle
    x = zeros(N+1, length(x0));
    ...
    ceq = [];
    for k=1:N  
        [cnew, ceqnew] = constraints(t0+k*T,x(k,:),u(:,k),(k)*T, params_vehicles, params_lane);
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
    
%     %%% print stage constraints
%     fprintf('stage constraints at current time step: \n');
%     fprintf('cnew: \n');
%     fprintf('%d \n', cnew);
%     fprintf('\n');
...
%     %%% print terminal constraints
%     fprintf('terminal constraints at current time step: \n');
%     fprintf('cnew: \n');
%     fprintf('%d \n', cnew);
%     fprintf('\n');
    ...
    %%% Added function here  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [cnew, ceqnew] = chance_constraints(x,u,T,id_vehicle, params_vehicles,number_vehicles,id_lane_vehicles,distance_detectable,N,risk_parameter,states_vehicles);   % Chance constraints to avoid collision
    c = [c cnew];
    ceq = [ceq ceqnew];
...
%     fprintf('nonlinear constraints at current time step: \n');
%     fprintf('%d \n', c);
%     fprintf('%d \n', ceq);
...
function Flag_Error_f=printSolution(system, printHeader, printClosedloopData, ...
